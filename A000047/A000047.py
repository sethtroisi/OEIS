...
def get_three_five_prime_counts(n, primes):
    ...
    # Share a pointer to the same counts array. This lets us interate V and counts at the same time
    V_and_c = []
    for v in itertools.chain((n//i for i in range(1,r+1)), reversed(range(1, n//r))):
        d,m = divmod(v, 8)
        V_and_c.append((v, [2*d + (m >= 1) + (m >= 7), 2*d + (m >= 3) + (m >= 5)]))

    C = {i : c for i, c in V_and_c }
...
def A000047_final(bits: int) -> int:
    ...
    if bits > 35:
        print(f"\tprimes({len(primes)}) {primes[0]} ... {list(primes[-3:])}")
